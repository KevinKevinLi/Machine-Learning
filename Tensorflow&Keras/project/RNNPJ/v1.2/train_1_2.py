...
def readTrain():
  train = pd.read_csv("980101190212_fac.csv")
  return train
...
def normalize(train):
  ...
  train_norm = train.iloc[:,0:7].apply(lambda x: (x - np.min(x)) / (np.max(x) - np.min(x)))
  train_norm = np.c_[train_norm, train["day"]]
  train_norm = pd.DataFrame(train_norm, columns=["Open","High","Low","Close","Adj Close","Volume","Next open","day"])
  return train_norm
...
def onehot(train):
  ...
  result = pd.DataFrame(result, columns=["Open","High","Low","Close","Adj Close","Volume","Next open","day","one","two","three","four","five"])
  return result
...
def buildTrain(train, pastDay, futureDay):
  ...
  for i in range(train.shape[0]-futureDay-pastDay):
    ...
    X_train.append(np.array(train.iloc[i:i+pastDay,[0,1,2,4,5,6,8,9,10,11,12]]))
